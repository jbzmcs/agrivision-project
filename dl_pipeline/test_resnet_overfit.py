...
# --- Configuration ---
DATA_DIR = 'data'
# --- CHANGED: New input/output paths ---
MODEL_PATH = 'models/resnet50_overfit.pth' 
OUTPUT_DIR = 'output/resnet_overfit'
# -------------------------------------
BATCH_SIZE = 64
...
def build_model(model_path, num_classes):
    ...
    try:
        ...
        print("Please run train_resnet_overfit.py first.")
        sys.exit(1)
...
def save_evaluation_metrics_table(avg_loss, acc_top1, num_classes, labels, probs, preds, k, output_dir, model_name="ResNet50 (Overfit)"):
    """
...
def plot_classification_report_heatmap(labels, preds, probs, class_names, num_classes, output_dir, model_name="ResNet50 (Overfit)", k=5):
    """
...
def save_confusion_matrix(labels, preds, class_names, output_dir, model_name="ResNet50 (Overfit)"):
    """
    ...
    plt.title(f"{model_name} Confusion Matrix (Test Set)", fontsize=18)
...
def save_roc_curves(labels, probs, class_names, num_classes, output_dir, model_name="ResNet50 (Overfit)"):
    """
    ...
    plt.title(f'{model_name} ROC Curve - Test Set', fontsize=16)
...
def save_pr_curves(labels, probs, class_names, num_classes, output_dir, model_name="ResNet50 (Overfit)"):
    """
    ...
    plt.title(f'{model_name} Precision-Recall Curve - Test Set', fontsize=16)
...
def main():
    ...
    """
    print(f"Starting ResNet50 OVERFIT Evaluation")
    ...
    # 4. Save Artifacts
    model_name = "ResNet50 (Overfit)"
    
    save_evaluation_metrics_table(
        avg_loss, acc_top1, num_classes, labels, 
        probs, preds_top1, k=5, output_dir=OUTPUT_DIR, model_name=model_name
    )
    
    plot_classification_report_heatmap(
        labels, preds_top1, probs, class_names, 
        num_classes, OUTPUT_DIR, model_name=model_name, k=5
    )
    
    save_confusion_matrix(labels, preds_top1, class_names, OUTPUT_DIR, model_name=model_name)
    save_roc_curves(labels, probs, class_names, num_classes, OUTPUT_DIR, model_name=model_name)
    save_pr_curves(labels, probs, class_names, num_classes, OUTPUT_DIR, model_name=model_name)
    
    print("\nResNet50 OVERFIT evaluation complete.")
