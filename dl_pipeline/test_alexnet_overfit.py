...
MODEL_PATH = 'models/alexnet_overfit.pth' 
...
def save_confusion_matrix(labels, preds, class_names, output_dir, model_name="AlexNet (Overfit)"):
    """
    ...
    plt.title(f"{model_name} Confusion Matrix (Test Set)", fontsize=18) # <-- CHANGED
...
def save_roc_curves(labels, probs, class_names, num_classes, output_dir, model_name="AlexNet (Overfit)"):
    """
    ...
    plt.title(f'{model_name} ROC Curve - Test Set', fontsize=16) # <-- CHANGED
...
def save_pr_curves(labels, probs, class_names, num_classes, output_dir, model_name="AlexNet (Overfit)"):
    """
    ...
    plt.title(f'{model_name} Precision-Recall Curve - Test Set', fontsize=16) # <-- CHANGED
...
def main():
    """
    Orchestrates the testing and evaluation process FOR THE OVERFIT MODEL.
    """
    print(f"Starting AlexNet OVERFIT Evaluation") # <-- CHANGED
    ...
    # 4. Save Artifacts
    model_name = "AlexNet (Overfit)" # <-- CHANGED
    ...
    # We must also update the titles in the other save functions
    ...
    print("\nAlexNet OVERFIT evaluation complete.") 
